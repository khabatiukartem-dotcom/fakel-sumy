...
const Header = () => {
  ...
  return (
    <>
      ...
      >
        <div className="container mx-auto px-4 md:px-8 max-w-7xl flex items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center gap-2 xl:gap-6">
            {navLinks.map((link) => (
              <a 
                key={link.name} 
                href={link.href}
                className="text-[var(--text-primary)] hover:text-[var(--accent-fire)] font-montserrat font-semibold text-sm uppercase tracking-wider transition-colors"
              >
                {link.name}
              </a>
            ))}
          ...
          <div className="flex items-center gap-4">
            <Button variant="primary" className="hidden lg:flex" onClick={() => document.getElementById('contact').scrollIntoView({ behavior: 'smooth'})}>
      ...
      >
        {/* Adds a slight visual texture to mobile menu */}
